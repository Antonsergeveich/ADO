...
namespace Academy
{
	internal class Connector
	{
		...
		public static Dictionary<string, int> LoadPair(string fields_name, string field_id, string tables, string condition = "")
		{
			Dictionary<string, int> dictionary = null;
			string cmd = $"SELECT {fields_name},{field_id} FROM {tables}";
			if (condition.Length > 0) cmd += $" WHERE {condition}";
			cmd += ";";
			SqlCommand command = new SqlCommand(cmd, connection);
			connection.Open();
